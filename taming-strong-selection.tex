...
\end{figure}
The set of selection transition probability matrices $\mathbf{T_{n_o,n_p}}$ requires $O(r_{max}n_o^2 n_p^2)$
operations to construct: Since each probability $P_{n_o}(\mathcal{S}_{n_o}(i_o, n_p, r)| \mathcal{C}(i_p,n_p) )$ can be computed from a recursion with
