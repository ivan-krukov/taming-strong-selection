...
remains challenging, and few studies have tackled this regime.

Since a large body of work aimed at understanding the distribution of fitness effects for new
mutations relies on the diffusion approximation (\textit{e.g.} \cite{EyreWalker2006, Boyko2009}), an approach
...
From a mathematical point of view, we treat the number of relevant lineages as we go back 
...
selective deaths is smaller than common ancestry events in sufficiently large samples, 
which would prevent the increase in the number of lineages. 
We then then show that this happens not just in expectation, but almost always, 
and provide a closure approximation for the asymptotically rare exceptions.
...
To disentangle the effects of drift and selection, we may also define $n_g$ as the number of gametes
...
offspring and contributing parental lineages, respectively. Such recursions under neutrality were studied in \cite{Lessard}.
...
$n_p$ parental alleles are derived.  The term $\epsilon$ accounts for events with $n_p>n_o.$ 

We will show below that $\epsilon$ is typically small in large sample sizes. Further, it can be 
approximated effectively using a jackknife approach \citep{Gravel2016}. 
Such jackknife approaches have been used in \cite{JouganousEtAl2017} to derive approximate 
recursion equations under weak selection. But whereas the approach of \cite{JouganousEtAl2017} 
require a jackknife approximation for every selective death, the formulation in Equation \eqref{eq_truncated}
only requires approximations in the (rare) instances where there are more selective deaths than 
common ancestry events. Thus larger sample sizes mean that fewer approximations are needed.


Our first goal is to compute the matrices in \eqref{eq_recur}. To do so, we
...
The recursion will be over $\mathbf{T}_{r}\Coalc{i_p}{n_p}{i_o}{n_o},$ which generalize the 
...
is convenient to keep track of the number of relevant parental lineages (at
...
\cite{BhaskarEtAl2014}) and analytical expressions \cite{Lessard} can be computed in $O(n_o^3)$. 
...
Figure \ref{fig_strong_selection} and supplementary Figure \ref{} shows the comparison of the \textit{AFS} calculated from Equation
\ref{eq_sfs_calc}, \sgcomment{This is not correct -- what is shown is the jackknife-corrected result, right?} 
...
Wright-Fisher and the diffusion solutions for singletons and doubletons even under neutrality. This effect increases with
sample size, and has been analyzed in \citep{Fu2006, BhaskarEtAl2014}. There is a also a strong divergence
