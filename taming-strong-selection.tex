...
approximation.\cite{GutenkunstEtAl2009, JouganousEtAl2017}).

Second, both Kingman's coalescent and diffusion-based models rely on an approximation that samples
studied are smaller than the square root of the effective population size, an increasingly
...
Spence2016}, and references therein), but accounting for natural selection remains challenging.
...
In this article we derive these transition matrices, and show that the additional effort in
bookkeeping for large sample sizes has the unexpected benefit of solving our first challenge by
making the recursions asymptotically closed.
...
sample sizes, and has been analyzed in \citep{BhaskarEtAl2014}. There is a also a strong divergence
...
Here, we see a more consistent difference between the models. An extended version of the figure can
be found in \ref{fig_apx_strong_selection_extended}.
...
\begin{figure}
  ...
  \caption{Relative error of allele frequency spectra in a sample of size $n=200$ with respect to
    ...
    With negative selection, the predictions differ substantially at modelrate allele frequencies.
...
small. To show this, we demonstrate the difference in total genetic load from \textit{AFS} from our
method compared to the diffusion \textit{AFS}, which is show in table \ref{tab_apx_load}. The total
...
largest differences are for very deleterious alleles, where the fixations are unlikely to begin
with. We only note that this might be relevant on the evolutionary time scales.
...
\begin{figure}[H]
  ...
  \includegraphics[width=\textwidth]{fig/afs_comp_big.pdf}
    ...
    \caption{Extended verison of figure \ref{fig_strong_selection}. Relative error of allele
    frequency spectra from different models with respect to the Wright-Fisher \textit{AFS}. Panels
    horizontally - population size ($N=2000,1000,200$). Note that the last column correposnds to the
...
contribute appreciably.  In addition, we need to build the matrices $T_{n_p', n_p}$ themsleves.  In
