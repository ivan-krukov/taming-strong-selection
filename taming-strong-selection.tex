...
\author{Ivan Krukov}
\author{Simon Gravel}
\affil{Genome Center, Department of Human Genetics, McGill University, Montreal, Canada}
\maketitle
...
and selection in that regime can be used to define asymptotically closed recursions for the
distribution of allele frequencies that are accurate well beyond the strong selection limit.

Selection becomes more analytically tractable when the sample size $n$ is larger than twice the
population-scaled selection coefficient: $n \ge 2Ns$ ($4Ns$ in diploids). That is, when the expected
number of coalescent events in the sample is larger than the number of selective events.  We
construct the relevant transition matrices, show how they can be used to accurately compute
distributions of allele frequencies, and show that the distribution of deleterious allele
frequencies is  sensitive to details of the evolutionary model.  
...
mutations (\textit{e.g.} \cite{EyreWalker2006, BoykoEtAl2008}). However, its validity does not extend to 
sample sizes used in recent studies that investigate selective constraints \citep{karczewski2020mutational}.
...
selective deaths can require the drawing of $n_p>n_o$ \citep{DonnellyKurtz1999a,
JouganousEtAl2017}. This possible increase in ancestral lineages is also made explicit,
for example, in the framework of the ancestral selection graph
...
We will construct this by considering the process of drawing parental alleles (at time $t-1$) for this finite
sample.
