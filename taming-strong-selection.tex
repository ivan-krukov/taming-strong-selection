...
\textit{AFS} under the Wright-Fisher model could be modified to include multiple-merger events and selection.  Here we derive these recursions and study the impact of large sample sizes on the validity of diffusion models.
...
\textit{AFS} at $t-1$. To do this, we can sum over possible values for random variables $n_p$ and the number $i_p$ of distinct derived alleles
among the parents:
\begin{equation}
...
on population size and selection coefficients. Concretely, we consider the event $(i_o, i_p, n_p)$ that $i_o$ offspring carry the deleterious allele, that $n_p$ distinct parents were drawn (successfully or not) when generating the $n_o$ offspring, and that $i_p$ of these $n_p$ parents carry the deleterious allele.  \sgcomment{Check throughout that we use `deleterious' allele consistently, vs 'derived'}

This event $(i_o, i_p, n_p)$ can be expressed as the intersection of two events, $\mathcal{C}(i_p,n_p)$ and $\mathcal{S}_{n_o}(i_o,
