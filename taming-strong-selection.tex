...
We consider a haploid Wright-Fisher model with $N$ individuals per generation, focusing on expectations for a single biallelic locus with a selection coefficient $s>0$. As usual in the Wright-Fisher model, each offspring is generated by drawing a parent at random from the parental population, with probability of drawing a given parent depending on the allele it carries: the probability of drawing a given parent with the deleterious allele is $(1-s)$ times the probability of drawing a given parent with the beneficial allele.  One way to generate such sampling probabilities that we will find mathematically convenient is to draw parents with equal probability but reject, with probability $s$, draws of parents carrying the deleterious alleles. Since each offspring does need a parent, offspring with a rejected parental draw get a new draw and the process is iterated until a successful parental draw. This iterative process is equivalent to the usual Wright-Fisher selection, as (successful) parental draws have the correct probabilities. It also generates "failed" draws that we can interpret as reproductive failures or early selective deaths among offspring. 

In principle, the number of rejections for each successful draw can be arbitrarily large, but this will happen with low probability for $s<1$. For numerical reasons outlined below, we will sometimes allow for only a finite number \sgcomment{...} of failed draws for each offspring (i.e., we will impose that the \sgcomment{...}$+1$th draw is always successful.) This modification to the standard Wright-Fisher model will be negligible for \sgcomment{...} large enough. 

Finally, we suppose that offspring receive the allele from their parent with probability $1-u,$ and a different allele due to mutation with probability $u.$ \sgcomment{This needs a bit of work}




For a present-day sample with $n_o$ offspring lineages at time $t$, we will be
...
We will construct this by considering the process of drawing parental alleles (at time $t-1$) for this finite sample.
...
To model selection as part of our sampling process, we consider that a selected parental lineage carrying a
derived allele can fail to reproduce with probability $s\ge0$ (Fig.
...
we will see that the \textit{AFS} in the offspring will depend on the state of the $n_p$ distinct parental lineages
that were sampled.
...
\begin{equation}
  \afs{n_o}{t} = \sum_{n_p}  \mathbf{T}_{n_o,n_p}     \afs{n_p}{t-1}
    \label{eq_recur}
  \end{equation}
