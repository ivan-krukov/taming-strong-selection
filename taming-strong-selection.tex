...
This is moderately more complex than in the neutral case, where recursions \cite{BhaskarEtAl2014}
 and analytical expressions \cite{Lessard2010} can be computed in $O(n_o^3).$
...
(i.e., with pure truncation, $\epsilon = 0$, Figure S\ref{fig_apx_no_jackknife}) show moderate departures from the exact results, 
...
as discussed in \cite{Fu2006}. The abundance of large families with the coalescent provides a higher probability for
...
\begin{figure}[H]
  ...
  \includegraphics[width=\textwidth]{fig/afs_comp_small_no_jackknife.pdf}
    ...
    \caption{Allele frequency spectra calculated without the use of the jackknife.    }
...
\begin{figure}[H]
  ...
  \caption{Probability that there are more contributing parents than offspring: $n_p > n_o$.
    Calculated as $1-\sum_{i_o} \mathbf{Q}_{n_o}{(i_p, i_o)}$, with $N=1000$. Each panel shows an
    increasing order of the jackknife approximation.}
