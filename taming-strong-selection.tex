...
A benefit of the uniform sampling with rejection approach is that parents are drawn independently of their genotype (i.e., they are exchangeable with respect to drawing, but not with respect to rejection). We can emphasize this property using another equivalent formulation of the Wright-Fisher model that is closely related to the lookdown model \cite{}. Since the $N_t$ offspring are generated independently, we can choose to generate them iteratively rather than simultaneously. Attempted parental draws in this iterative process form a sequence of parents drawn uniformly with replacement. It is easy to show that a sequence $S$ drawn uniformly with replacement from a set $\Omega$ can be generated in two steps. First, we draw a random permutation $\sigma(\Omega).$ This will define the order of first sampling of elements of $\Omega$ in $S$. As we proceed through the sampling of $S$, we simply keep track of highest index $k$ of a sampled element in $\sigma_{\Omega}$, and draw either a previously sampled element from $\{\sigma\}_{i,1,\ldots, k}$, each with probability $1/|\Omega|$, or element $\sigma(\Omega)_{k+1}$ (with probability $1-k/|\Omega|$). We therefore first choose a random permutation of the parents, generate offspring in turn, and allow each offspring to attempt choosing parents either from parents previously drawn, or from the next parent in the permutation. Selection is implemented as above. 
This approach is strongly reminiscent of the lookdown model \cite{}, and we will refer to it as the lookdown construction. 
...
\begin{figure}
\scalebox{0.45}{\includegraphics{./fig/reversible_bottleneck_comparison.pdf}}
...
\\
    $\mathcal{S}_{n_o}(i_o, n_p)$ &  $\ms_{n_o}(i_o, n_p, r=0)$. Equivalently, $\mathcal{F}_{n_o}(i_o) \cap \mathcal{D}_{n_o, 0}(n_p).$ %: Event that $i_o$ out of $n_o$ offspring carry the derived allele and 
      %that $n_p$ distinct parental lineages were sampled while generating the $n_o$ offspring (including rejected parental lineages).  
      

      %Event that  $i_o$ out of $n_o$ offspring carry the derived allele, that $r_{n_o+1} \geq r,$  
    %and that $n_p$ distinct parental alleles were drawn up to the $r$th failure for drawing 
    %individual $n_o+1$   
    \\
