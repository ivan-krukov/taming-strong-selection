...
One way to generate such sampling probabilities is to randomly draw parents with equal probability but reject, with probability $s$, draws of parents carrying the deleterious alleles. We can think of such rejected draws as caused by reproductive failures.   
For each offspring, we repeat the random parental draw until a parent is assigned, that is, 
until a draw occurs without rejection (Fig. \ref{fig_schematic}B, broken lines). 
This iterative process is equivalent to the usual Wright-Fisher sampling, as each parent has the correct probability of being assigned to each offspring. To generate $n_o$ offspring, we will make $n_g$ distinct draws ($n_g\geq n_o$ since there can be rejected draws), corresponding to $n_p$ distinct individuals in the parent generation ($n_p\leq n_g$ since the same parent can be assigned more than once, in which case we say that there was `shared ancestry' or `coalescence').  
...
The offspring are assigned the parental allele, unless a mutation occurs. We consider different mutational models below, and first consider a model where mutations take place in the offspring after the parental allele has been copied. 

When we consider a sample from this population, we sample immediately after this mutation step.  
