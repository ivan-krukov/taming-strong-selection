...
differences under selection span the entire distribution of allele frequencies (Figure \cite{}). 
Such discrepancies point to differences in the distribution of gene genealogies that go beyond 
details whether coalescence events are simultaneous or sequential within a generation. 
...
the differences in sibship sizes are not uniform: There are a bit more sibships of size 1 and many more families of size above 4 
...
The larger differences in prediction of allele frequencies between Wright-Fisher and Kingman's 
...
\cite{Lessard} has derived recursions for a wide range of neutral models, and we expect that many of those 
can be generalized to almost-closed recursions with selection. 

Finally, even though we focused here on simple demographies, many methods for the analysis of complex demographies, 
such as moments and momi, build on discrete transition matrices for the SFS. The existence of robust transition matrices 
under selection opens up the possibility of extending these methods to a wider range of applications. 

A remaining challenge of the present approach is that the computation of the transition matrices is itself costly. 
It can be pre-computed, but this limits the applicability of the approach to step-wise constant population sizes 
