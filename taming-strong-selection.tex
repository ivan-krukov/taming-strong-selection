...
However, current studies challenge this assumption (e.g., \citep{karczewski2020mutational} with $n_o=...$). Holding on to the incorrect assumption leads, among other problems, to an incorrect estimation of the proportion of rare variants in large samples \citep{Fu2006, BhaskarEtAl2014}. 
...
Here we propose a recursion approach to computes the expected AFS at one generation from the expected AFS at a previous generation. Iterating the recursion from a distant past to the present provides expected AFS in large samples under selection. Since recursions equations have a long history in population genetics, this approach has many relatives. It is similar in spirit to the diffusion equation, in that it tracks distributions of allele frequencies forward in time, but it bypasses the classical diffusion limit. Recursions derived without the diffusion limit include the classical recursion for the evolution of heterozygosity under neutrality \cite{}, and have been generalized to a range of finite-sample models under neutrality \cite{Lessard2010,JouganousEtAl2017,others}. 
