...
The manuscript is organized as follows. We first provide a formulation of the Wright-Fisher model that treats selection as a discrete, stochastic process.   
...
To select a parent with the specified probabilities, we use a rejection scheme: We first draw a parent uniformly at random, but reject the draw with probability $s.$ For each offspring, we must then iteratively draw parents until a draw is accepted.  
%This sampling scheme is clearly equivalent to the usual Wright-Fisher model. It allows us to treat parents as exchangeable under the initial draw, and to treat selection as a discrete stochastic process that is hidden in the usual formulation of the Wright-Fisher process. 
...
%    triggering a redraw. Depending on the number of selective failures and drift events, $n_p$ can be smaller than, equal, or larger than $n_o$.   }
%
%\end{figure}
...
To obtain a recursion on the \textit{AFS}, we will consider $n_o$ individuals 
that were drawn at random from the offspring population. 
Since the offspring are generated independently in the Wright-Fisher process,
the order in which they are created does not matter, and 
we can imagine that these $n_o$ offspring were created iteratively (i.e., one at a time). 

The sequential sampling of the $n_o$ offspring then involves a sequence of $n_g$ parental draws, of which $n_g-n_o$ are rejected. 
Each parental draw is taken uniformly from the parental generation. Thus our formulation
of the Wright-Fisher process for a sample of $n_o$ offspring involves drawing a sequence 
of parents uniformly at random, with replacement, until a condition is met 
(i.e., until we had $n_o$ successful draws). 
This sequence of 


Since parents are exchangeable with respect to the sampling process, 
each previously unsampled parent is equally likely to be sampled at each step.  
Rather than deciding on which unsampled parent 

We can therefore generate a random permutation of the parents before , 
and draw previously unsampled parents in order from this permutation. 
This allows us to separate random properties of this permutation,
which depend only on parental \textit{AFS}, from properties of Wright-Fisher sampling given a permutation, 
which depend on population size and selection coefficients.

Concretely, consider a random sample of size $n_o$ in the offspring generation and the event $(i_o, i_p, n_p)$ that $i_o$ offspring 
in the sample carry the deleterious allele, that $n_p$ distinct parents were drawn (successfully or not) when generating the $n_o$ offspring, and that $i_p$ of these $n_p$ parents carry the deleterious allele is true if and only if two distinct events are true. 
First, it requires that $i_p$ of the first $n_p$ parents in the random permutation of the parental population carry the deleterious allele 
(we call this event $\mathcal{C}(i_p,n_p))$. Second it requires that the
Wright-Fisher process to generate $n_o$ offspring required drawing exactly $n_p$ parents and copied the derived allele into 
$i_o$ of the offspring (we call that event that event $\mathcal{S}_{n_o}(i_o, n_p)$).
...
Appendix \nameref{subsec_apx_tpm_deriv}), given event $\mathcal{C}(i_p,n_p)$ that $i_p$ of the first
...
%The recursion will be over a probability
%$T_{r}\Coalc{i_p}{n_p}{i_o}{n_o}$ which generalize the transition matrices from Equation
%\eqref{eq_truncated} to account for rejected draws: $\mathbf{T}_{n_o,n_p}\equiv
%T_{r=0}\Coalc{i_p}{n_p}{i_o}{n_o}$. Formally,  $T_{r}\Coalc{i_p}{n_p}{i_o}{n_o}
...
present method is exact. In this case, however, the recursion could be obtained much more simply by noticing that the allele frequency in the offspring generation can be obtained by binomial sampling from the frequency in the parental population.  The present approach is thus useful where accurate recursions can be
...
expect drift to almost always overtake selection.  Appendix \nameref{subsec_apx_gauss} shows that the sample size
...
(see Appendix \nameref{subsec_apx_multi}). If the expectation of $n_p-n_o \simeq n_o  s - \frac{n_o (n_o-1) }{2N}$ is negative, the probability that we require more than $n_o$ ancestral lineages $g$ generations ago decreases with $g.$
...
%    $i_o \dslash n_o$ & A sample with $i_o$ derived alleles ``out of'' $n_o$ total lineages\\
    ...
    $\mathbf{T}_{r}\Coalc{i_p}{n_p}{i_o}{n_o}$ &$P_{n_o}\left[\ms_{n_o}(i_o, n_p, r) | \mathcal{C}(i_p, n_p)\right]$. A central quantity as it captures the roles of drift and selection in recursion \eqref{eq_recur}. 
  \end{tabular}
