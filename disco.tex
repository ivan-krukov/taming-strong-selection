...
negative selection -- due to selective deaths, the number of parental lineages that are involved can
be larger than the sample size, $n$. The possible increase in the number of relevant lineages as we go
back in time is explicit, e.g., in the ancestral selection graphs (\textit{ASG}),
\citep{KroneNeuhauser1997}, and was discussed in [cite Kurtz and donnely on selection].
...
Large sample sizes bring in the complication of multiple and/or simultaneous coalescent events \citep{BhaskarEtAl2014}. 
The standard coalescent model only allows pairwise common ancestors, but we also need to consider higher-order events, such as 
three-way coalescence. These multiple-lineage coalescent events oppose the effect of selection by
...
Under neutral evolution, the number $n_p$ of distinct parental lineages drawn in this process is smaller than
...
can be ``rejected'' as a result of selective death with probability $s>0$
