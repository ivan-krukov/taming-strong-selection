...
$P_{n_o}\left[\mathcal{Q}_{n_o}(i_o,r), \ms_{n_o,r}(n_p) | \mathcal{C}(i_p, n_p)\right]$ of two events conditional on 
$\mathcal{C}(i_p, n_p)$: the event $\mathcal{Q}_{n_o}(i_o,r)$ that exactly $i_o$ of the
first $n_o$ successfully drawn offspring are derived, and that the following $r$ draws are rejected, 
and the event  $\ms_{n_o,r}(n_p)$ that $n_o$ successful draws and $r$ failures require exactly $n_p$ 
distinct parental draws. \sgcomment{do we ever separate Q and S? If not merge?} 


- Figure \ref{fig_rec_selection_dynamic_fail}. This bracket notation is convenient to keep track of
...
% add a sentence about normalizing
...
\ikcomment{Better notation}
\ikcomment{Better normalization}
...
\ikcomment{Change the quantiles}
...
\end{split}
\end{equation}
where $n'_{p'}$ is the number of sampled lineages in the grandparental generation, and $n_{p,max}$ is the maximum number of 
sampled parental alleles considered. 
