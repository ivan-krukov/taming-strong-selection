...
The interplay of drift and selection is important to consider. The number of
coalescent events is proportional to the sample size, while the number of
selective deaths is independent from it. This suggests that with sufficiently
large samples, the effect of selection would be smaller than that of drift,
which would lead to almost-closed recursion equations, as we will demonstrate
here.
...
To estimate the allele-frequency spectrum $\afs{n_o}{t}$ in a sample size $n_o$
at time $t$, we can sum over the random variables $n_p,$ the number of distinct
...
\afs{n_o}{t}(i) = \sum_{n_p,j} P(i | r(j,n_p), n_p)  P(n_p | r(j,n_p) ) P(r(j,n_p)) 
    = \sum_{n_p,j} P(i,n_p | r(j,n_p))  \afs{n_p}{t}
\end{equation}
