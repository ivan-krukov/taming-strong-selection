...
In this model, there can be an arbitrarily large number of selective failures for each successful draw. For practical reasons, we will limit the number of failed draws for each given offspring to a finite number $k$. To ensure that probabilities remain well-defined, we thus modify the Wright-Fisher model so that the $k+1$th attempted draw for a given offspring cannot end in a selective death. We must then choose $k$ large enough to ensure that the modified sampling is rarely used.
...
The sample size $n$ can range between $1$ and $N$, the size of the entire population. We expect that 
...
$n \ra N$, we expect the transition probabilities in \sgcomment {Call this matrix something else? R} $\mathbf{P}_s$ to approach those in the full
