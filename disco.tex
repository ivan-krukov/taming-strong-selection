...
\begin{table}
  ...
  \begin{tabular}{l|p{100mm}}
    ...
    $\mathbf{T}_{r}\Coalc{i_p}{n_p}{i_o}{n_o}$ & Probability of transition from $i_p \dslash n_p$ in parents
                                                 to $i_o \dslash i_p$ in offspring, with $r$ selective events\\
  \end{tabular}
...
Even though $\mathbf{T}_{n_p,n_o}$ is a simple combinatorial probability describing a single generation, we
...
size $n_o$, by assuming that we know $\mathbf{T}_{n_p,n_o-1}$ conditional on the last drawn offspring.
...
The term $\mathbf{T}_{r}\Coalc{i_p}{n_p}{i_o}{n_o}$ is an entry in $\mathbf{T}_{n_p,n_o-1}$, and
represents the probability of drawing $i_o$ derived alleles into the offspring generation from $i_p$
derived in the parental generation, with exactly $r$ selective events for that lineage. $n_p$ and
$n_o$ are the sample sizes of parental and offspring generations, respectively.
...
$\mathbf{T}_{r}$ is the sum of terms \textit{rC} and \textit{rD}.
...
Once the truncated matrix $\mathbf{Q}_{n_o}$ is constructed, it can be used to calculate the allele
...
\begin{figure}
  ...
  \caption{Normalized allele frequency spectra in a sample of size $n=100$, for neutral (Ns=0)
    ...
    ($N=100$). (D,E,F) same, for deleterious allele. Note that for a strongly deleterious allele,
...
closed \eqref{eq:truncated}. We derive several approximations to the model proposed in the first
...
number of rejected lineages, we take $x=1$, similar to \ref{subsec:missing}.

In addition, we now explicitly consider the number of gametes produced by the parents - Fig.
\ref{fig:schematic}C. The parental generation ($n_p$), produces a large number of gametes, of which $n_g$ are randomly picked to contribute to the offspring. Each derived allele is then passed to one offspring with probability $1-s$, ancestral with probability $1$. If picking a gamete fails, with probability $s$, we sample a new one.

Here, the number of coalescent events is $n_g - n_p$, and the number of selective deaths is
$n_g - n_o$.
...
Since $n_g-n_o$ follows a negative binomial distribution with $n_g$ trials and success rate $1-s$,
we can use the moment generating function for any constant $k$,
...
This gives us a convenient expression for the critical sample size, such that there are
more drift than selection events, on average. In the following section, we derive the distribution of the number of contributing lineages one generation into the past. In this manner, we can find the critical sample size so that $n_p \leq n_o$ almost always.
...
The distribution of the number of extra lineages contributing from the parental generation is derived in section \ref{subsec:apx:variance} of the appendix.
...
a sample $n=200$. In the absence of selection, the distribution has zero probability above $n=200$, as
...
We use the mean and variance calculated in \ref{eq:lineages-exact} and \ref{eq:apx:var} respectively. 
...
(assuming all alleles are derived). If we want to ascertain that the distribution is closed with
increased confidence, we require a larger number of lineages. All curves assume $N=1000$.

Figure \ref{fig:apx:ciritical-fraction} shows the critical sample size as a fraction of the total
population size.
