...
The number of in-sample coalescent events per generation is proportional to the square of the sample size $n_o$,
...
\citep{BhaskarEtAl2014}. This makes the computation of transition matrices for recursion equations
more difficult, but not impossible. In this article we derive these transition matrices, and
...
the selection effects. We show that these transition matrices can be used to accurately model the 
...
have been drawn. Event $\mathcal{C}_{n_p}(i_p)$ is a property of the allele frequency distribution 
in the parental generation, 
and in fact $P(\mathcal{C}_{n_p}(i_p)) =\afs{n_p}{t} (i_p)$, by definition of the \textit{AFS}. Thus \sgcomment{Notation inconsitent
$\mathcal{C}_{n_p}(i_p)$ vs $\mathcal{C}(i_p,n_p)$}
...
These two recursions, and using caching to avoid re-computing previously computed values,
