...
To \sgcomment{show-investigate} the closure properties of $Q$, we can calculate the total probability that more that $n$
parental lineages contribute to the sample of a given size.
By construction, the sum of rows of $Q$
should correspond to the total probability mass that included configurations contribute (Fig.
\ref{fig:recurrence} \sgcomment{no figure}). Thus, the probability that some number of configurations are unaccounted for,
...
or drift \sgcomment{I don't know what that means}. Then, we construct a full probability distribution of the number of contributing lineages
...
The upper bound \sgcomment{which upper bound?} on the number of lineages used is of particular interest - it represents the worst
...
This expression is not hard to derive intuitively \sgcomment{You rarely gain anything by pointing out that something is easy or intuitive}. First, the probability of selecting a particular
...
\citep{KroneNeuhauser1997}, eq. \eqref{eq:asg-size}, but now includes sample size directly \sgcomment{I don't understand why the sample size is included more directly. It seems like we have spent a lot of space to re-derive well-known results.}.
...
which we denote as $n_{t-1}$:  \sgcomment{Why are you doing this?}
...
the starting sample size, we converge to the equilibrium relatively quickly. \sgcomment{This section was way too long. It could be summarized in two sentences.}
...
\sgcomment{Don't leave a space after eq if in same sentence to avoid indent} where $S_2(g,r)$ is a Stirling number of the second kind, which is the number of ways to partition
...
\eqref{eq:drift-expectation}. \sgcomment{too much detail here}

The occupancy distribution is not simple to evaluate, but good performance can be achieved by
pre-computing a table of reduced occupancy numbers, using the algorithm of \cite{ONeill2019}. \sgcomment{It will not be clear to readers why you are mentioning that here.}
...
\sgcomment{This section is also too long. "The distribution of the number $g$ of gametes sampled to generate $n$ offspring under selection is a negative binomial EQN. The distribution of the number $r$ of distinct parents used to generate $g$ gametes under drift is the occupancy distibution[CITE] EQN. The chain rule gives us EQN  }

\sgcomment{As is, people will get lost in the details of the descriptions, even if they could have guessed the results themselves.}
...
Unfortunately, this distribution does not have a simple analytical form \sgcomment{How do you know?}. In certain parameter
regimes, this can be approximated by the normal distribution \citep{JohnsonEtAl2005,ONeill2019},
which we describe in the next section. \sgcomment{Wait until then to mention it! You are doing too much back in forth. Present equation, show the result, then talk about approximation. No need to foreshadow something that is coming in 5 lines and that no one is expecting. }
...
($Ns=10$, \eqref{eq:critical-sample}), the distribution is symmetric \sgcomment{it is not}.
...
We note that \sgcomment{"We note that" adds no information} at the critical sample size, the probability that we will have a sufficient number of
lineages is only $50\%$ \sgcomment{I don't see this}. In order to guarantee that drift will out-pace selection, we can calculate
