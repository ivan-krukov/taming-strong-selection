...
This is more complex than in the neutral case, where recursions can be computed in $O(n^3)$ as in \cite{BhaskarEtAl2014}.
...
(i.e., there is no numerical benefit to consider a subsample that is as large as the entire population). 
...
\sgcomment{This must be about correct, but am not sure about the transition from line 3 to line 4. }
where the sum over $i_p$ is part of the product of the transition matrices $T$. Since each matrix
product takes $O(n_o n_p n_{p'})$, and there are at most $O(n_p)$ such products, the computation of this
matrix takes at most $O(n_o n_p^2 n_{p'})$. In addition, we need to build the matrices $T_{n_p', n_p}$. 
