...
\begin{frontmatter}
  ...
  \begin{abstract}
    ...
    asymptotic results to determine the when the sample size is sufficiently large for drift to
...
simulations can be used to obtain a predicted \textit{AFS} \sgcomment{citations}. By comparing
...
size. Then to describe the a sample of size $n$, we need to recursively consider samples of size
$n'\le n$. This does not hold under negative selection -- due to selective deaths, the number of
parental lineages $n'$ can be larger than $n$. As we demonstrate later, this leads to a potentially
infinite number of terms in the equations.

This can similarly be framed in coalescent terms \citep{Kingman1982a}: the number of lineages can
only decrease as we go back in time, because of common ancestor events. By contrast, in the
ancestral selection graph (\textit{ASG}), \citep{KroneNeuhauser1997}, the number of relevant
lineages can also increase as we go back in time, because of selective death events.

The interplay of drift and selection is important to consider. In large sample sizes, there
are many more common ancestry events than selective deaths, and the number of relevant lineages is
unlikely to increase as we go back in time. This suggests that large sample sizes can lead to
almost-closed recursion equations, as we demonstrate here.
...
three-lineage mergers. These multiple-lineage coalescent events oppose the increase of
contributing lineages due to negative selection.

In this article we derive such almost-closed recursions in the Wright-Fisher model, and study their
behavior and applications for modeling the distribution of allele frequencies under strong
selection.
...
lineages ($n_p$) have been sampled from time $t-1$ (\ref{fig:schematic}). Under a neutral coalescent
model (\ref{fig:schematic}A), the number of contributing parental lineages at $t-1$ is
$n_p \le n_o$, as the number of lineages decreases backward in time.

To model the interplay of selection and drift, we consider a two-stage selection scheme
...
Under neutrality (\ref{fig:schematic}A), we have $n_p \in [1, n_o]$, therefore:
...
generation as a neutral process. Changing the subscripts in \ref{eq:op-neutral} to refer to
...
(\ref{eq:op-selection}) is no longer closed.
...
(\ref{fig:schematic}). The number of significant terms in the inner summation, however, depends on
...
\ref{eq:op-selection} decreases due to a large number of coalescent events, and smaller number of
selection resamplings.
...
is described by a birth-death process \citep{KroneNeuhauser1997, Wakeley2009}.
