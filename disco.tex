...
derived allele is $1-s$ times the probability of drawing a copy of the ancestral allele. Because the drawing
...
only on coalescence events. We will use this notation with $n_g$ in section
...
the event $(i_p,n_p)$ can be expressed as the intersection of two events, $\mathcal{C}(i_p,n_p)$
...
The AFS for $n_p\leq n_o$ can be obtained by downsampling from $\afs{n_o}{t-1}$ (\textit{i.e.}, $\afs{n_p}{t-1} =
...
A jackknife approximation \citep{Gravel2016} \sgcomment{The reference is misinterpreted by bibtex} can be used to simulate the drawing of a small number
...
Even though $\mathbf{T}_{n_p,n_o}$ Eq. \ref{eq:recur} is a combinatorial probability
...
arbitrary number ($r$) of selective deaths. \sgcomment{Previous paragraph could be simplified a bit}
...
Figure \ref{fig:rec-selection-dynamic-fail} (cases \textit{rC,rD}). To obtain $r$ selective deaths, we
must first have obtained $r-1$ selective deaths, followed by an additional selective death. We
must track whether the last failure was from a previously drawn parental lineage (case
\textit{rD}), in which case the number of parental lineages is unchanged, or of a new lineage (case
\textit{rC}), in which case the sample of size $n_o-1$ must have had exactly one fewer parent and 
...
$\mathbf{T}_{r}\Coalc{\cdot}{\cdot}{\cdot}{n_o-1}$. We can do this by conditioning on the last
...
A last barrier to overcome to compute the transition matrices is that  
the number of rejected lineages $r$ for individual draws can be infinite in the Wright-Fisher process.
...
$s<1$, we can easily pick $r_{max}$ to ensure excellent convergence. We found empirically that 
$r_{max}=4$, was amply sufficient to consider strong negative selection up to $Ns=50$ 
\sgcomment{Here perhaps give an example in terms of $s$ and the number of remaining lineages: 
For example, with $s=0.01$, which corresponds to $Ns=100$ with $N=10 000$, the probability of having more than 
four selective deaths is less than $10^{-10}$}.
...
operations to construct, as it implicitly constructs all the matrices for smaller sample sizes \sgcomment{Perhaps give a bit more detail? 
Since each term $\mathbf{T}_{r}$, r>0  in the recursion requires a constant number of operations, 
the computational complexity depends on the number of terms we need to calculate. We will need to compute terms of the form 
 $\mathbf{T}_{r}\Coalc{i_p'}{n_p'}{i_o'}{n_o'}$, with $1 \leq n_o'<n_o$, $i_o' \leq n_o'$, $0\leq i_p'$, and $0\leq n_p' \leq \alpha n_o$, 
 and $0\leq r \leq r_{max}.$ The terms  $\mathbf{T}_{0}$ have a bound of the same form (i.e., there are $O(n_o^4)$ terms, each requiring $r_{max}$ computations.  
 If we assume that $\alpha$ is order 1, the number of required terms is at most $O(r_{max} n_o^4)$.   }. In
the simpler neutral case, it is possible to calculate these matrices in $O(n^3)$,
similar to \cite{BhaskarEtAl2014} \sgcomment{"Similar to" or "as in"?}.
...
The recursions presented here provide excellent agreement with the analytical result even 
where the diffusion-based approaches depart considerably from the exact result. 

When the sample is the entire population, as in panel F, the present method is not needed since
exact recursion equations can be obtained from the binomial distribution (also not that in this case, $n_p\leq n_o$). 
...
$\mathcal{S'}_{n_o}$), given event $\mathcal{C'}_{n_p}(i_p)$ that $i_p$ of the first
...
This critical sample size (Fig.
\ref{fig:apx:critical-normal}), is where there are more drift than selection events, on average. 
...
calculations using the moment generating function are provided in the appendix, and the result is:
...
In a large sample with $1\ll n_o \ll N,$ we can think of the $n_p-n_o$ as the result of $n_o$ steps of a random walk. 
Each draw of a new sample defines a step, corresponding to the number of lineages gained or lost compared to the 
infinite-population size and no selection model. The step size can be zero (if no coalescence or selective death occurs), $-1$ (if a coalescence 
to a previously drawn lineage occurs), $+k$ (if $k$ selective deaths and no coalescence occurs), etc. When $n_o \ll N$ and $s\ll1,$ 
most steps are zero, and we can think of steps as approximately independent of each other.   

The selective deaths occurring for each drawn offspring are drawn completely independently, and 
the coalescence events are almost independent. 

Given a mean and variance, we can use a Gaussian distribution to approximate the distribution of the sampled parental lineages:
...
Figure \ref{fig:combined}A shows the excellent agreement exact distribution as dots, and the Gaussian approximation as
...
For the approximation to work well, we require \sgcomment{It not up to us :)} that the sample size is sufficiently large with
...
\sgcomment{It would be worth explaingin why/whether this is a reasonable criterion. 
Would this mean that we are loosing 1\% of probability at each generation?
 }
...
invalid: $\lambda < 10$. This shows that the truncation can be accurate even for strong selection 
and sample sizes much smaller than the full population size. The required sample size as a function 
