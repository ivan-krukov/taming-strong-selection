...
here can be used to draw a new lineage only when drift fails to the task. Concretely, we have
\begin{equation}
\begin{split}
\afs{n_o}{t} = \sum_{n_p}  \mathbf{T}_{n_p,n_o}     \afs{n_p}{t-1} &= \sum_{n_p\leq n_o }  \mathbf{T}_{n_p,n_o}   \afs{n_p}{t-1} + \sum_{n_p> n_o }  \mathbf{T}_{n_p,n_o} \afs{n_p}{t-1} \\
&=\mathbf{Q}  \afs{n_o}{t-1} + \sum_{n_p> n_o }  \mathbf{T}_{n_p,n_o} \afs{n_p}{t-1} 
\end{split}
\end{equation}
Here we have shown that setting the second term to zero can lead to asymptotically accurate computations. 
A jackknife approximation of the form $\afs{n_p}{t-1} \simeq J_{n_p, n_o} \afs{n_o}{t-1},$ as in \cite{Gravel2016}, 
