...
equations for the allele frequency spectrum ($\afs{n_o}{t}$) by considering the sampling process in
a finite sample under drift and selection. We define the \textit{AFS} as a probability of observing
a given allele frequency for a single locus.
...
allele are rejected with probability $s$, triggering a re-draw from another parent
...
number of coalescent events. Thus $n_p$ can be smaller than $n_o$ (if there is more
drift than selection), or larger (if there is more selection than drift). 

To express the allele-frequency spectrum $\afs{n_o}{t}$ for $n_o$ offspring in terms of the AFS at
time $t-1$, we can sum over the random variables $n_p$, the number of distinct parental lineages
selected, and $j$, the number of derived lineages among them:
...
where the subscript $n_o$ indicates probabilities that depend on $n_o$. The event $(j,n_p)$ means
that our Wright-Fisher sampling for $n_o$ offspring selected exactly $n_p$ distinct ancestors, of
which $j$ are derived. Under Wright-Fisher sampling, the order in which (previously unsampled)
...
order from this permutation. Thus the event $(j,n_p)$ can be reformulated as the joint events that
the first $n_p$ parental alleles from the random permutation carry $j$ derived alleles \textit{and}
that exactly $n_p$ distinct parents were drawn in the Wright-Fisher sampling of the first $n_o$
offspring. Now, let $r(j,n_p)$ be the (less specific) event that the first $n_p$ parental alleles
from the random permutation carry $j$ derived alleles. Then $P(r(j,n_p)) =\afs{n_p}{t} (j)$, and we
can write:

\begin{equation}
\begin{split}
  \afs{n_o}{t}(i)
  &=      \sum_{n_p,j}                                  P_{n_o}(i|r(j,n_p),n_p) P_{n_o}(n_p|r(j,n_p)) P(r(j,n_p)) \\
...
Under neutral evolution, $n_{p,max}\leq n_o$. Since we can obtain smaller \textit{AFS} from larger
\textit{AFS} through downsampling (\textit{i.e.}, $\afs{n_p}{t} = \mathbf{H}_{n_p,n_o} \afs{n_o}{t}$
for hypergeometric projection matrix $\mathbf{H}_{n_p,n_o}$ if $n_p<n_o$), Equation \eqref{eq:recur}
...
Under selection, $n_{p,max}$ can be larger than $n_o$, leading to an infinite set of coupled
equations. Our goal here is to take advantage of the fact that selective events can be treated
exactly in the large sample size limit as long as $n_p < n_o$ or, equivalently, as long as there are
more coalescent than selection events: $n_r \leq n_c$. If we have high confidence that
this will be the case, we can restore closure by truncating Eq. \eqref{eq:recur}:
...
Figure \ref{fig:selection-dynamic-failures} shows the recursion equation in the selection case. On
the right, filled circles correspond to derived alleles, empty to ancestral alleles. A line
connecting two circles represents a successful draw, a capped broken line - selective event. Square
brackets represent the events in a smaller ($n_o-1$) sample size. Double line represents potentially
multiple draws.

The term $T_{r}\Coalc{i_c}{n_c}{i_o}{n_o}$ represents the probability of drawing $i_o$ derived
alleles in the offspring generation from $i_c$ derived in the parental generation, with exactly $r$
selective events. $n_c$ and $n_o$ are the sample sizes of parental and offspring generations,
respectively.

Each of the summands (A,B,C,D) represents a particular type of a draw, whereas the recursive term
describes the events in a smaller sample size. To account for potentially multiple selection events,
we set
$T_{r=0}\Coalc{\cdot}{\cdot}{\cdot}{n_o} = \sum_{r=0}^{r_{max}}
T_{r}\Coalc{\cdot}{\cdot}{\cdot}{n_o-1}$. In words, the probability of no selective redraws in
sample size $n_o$ considers between $0$ and $r_{max}$ selective re-draws in sample size $n_o-1$.

To
account for multiple selection events, we add an additional parameter $t$, corresponding to the
number of selection re-draws. The term $T_{r}\Coalc{i_c}{n_c}{i_o}{n_o}$ represents the probability
of drawing $i_o$ derived alleles in the offspring generation from $i_c$ derived in the parental
generation, with $r$ selective events. The sample sizes are $n_o$ for the offspring, and $n_c$ for
the parents. We note that
...
Each
of the summands (A,B,C,D) corresponds to a lineage drawn in a sample size $n_o$, conditional on the draws in
sample $n_o-1$. To account for multiple selection events, we 


The derivation itself is somewhat tedious, and presented in Appendix X. \sgcomment{Maybe
  some of this should come back in this section? E.g. one of the plots?}
...
operations. As describe abo

\sgcomment{We will discuss approximate solutions below? add the number of selection events allowed per ploid, and add this in the scaling?}
