...
%Intuitively, the number of pairs of lineages that can coalesce grows quadratically with $n_o$, 
%whereas the number of lineages that can experience selective death grows as $n_o f_p s,$  
 %with $f_p$ the parental deleterious allele frequency. We expect many more coalescence than
 %selection events for sufficiently large sample sizes.  Using selection and coalescent rates from the 
%ancestral selection graph  \citep{KroneNeuhauser1997}, we can expect to find more drift than selection 
%events when $n_o> 2Nf_p s.$ 
...
\sgcomment{Use same notation as above?}
...
We can model the expectation $E[n_p-n_o | n_o]$ as the sum of lineages
lost to drift plus the number of extra
lineages required by selection

\begin{equation*}
  \begin{aligned}
    \label{eq:lineages-approx}
    E[n_p -n_o | n_o] &= E[n_p-n_g | n_o] + E[n_g-n_o | n_o] \\
    &= \sum_{n_g} E[n_p-n_g | n_g] P(n_g|n_o) + n_o\left( \frac{xs}{1-xs}\right)
  \end{aligned}
\end{equation*}
The first term represents the number of lineages lost to drift, and each expectation is bounded below by $E[n_p-n_o| n_o]$, so that we can write 
\begin{equation*}
  \begin{aligned}
    \label{eq:lineages-approx}
    \hat{E}[n_p -n_o | n_o] & \leq   N\left[1-\left( 1 - \frac{1}{N} \right)^{n_o} \right] -n_o + n_o\left( \frac{xs}{1-xs}\right)\\
     &\underset{N\gg n}{\approx} \frac{n_oxs}{1-xs} - \frac{n_o^2}{2N}
  \end{aligned}
\end{equation*}
...
%is much smaller than the population size. 
...
To ensure approximate closure of the recursion, we need a stricter constraint, we need $n_p\leq n_o$
not in average, but almost always. WE therefore need to study the distribution $P(N_p|n_0)$. 
