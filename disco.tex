...
In this work we show that increasing the sample size has another beneficial consequence. 
...
population sizes and selection coefficients. It is likely that computational 
speed ups, possibly involving approximations, can be derived for such cases.  


Closure of recursions require the simulated sample size to be above a critical size of $2N s.$ 
Below this threshold, the number of relevant lineages tends to grow as we go back in time. 
Above this threshold, the number of lineages tends to decrease, and it decreases with high 
probability if we are willing to wait a few generations. 

A limitation of the present approach is that it is relevant in a rather specific parameter regime, 
roughly $\sqrt{N}\leq n \ll N$:  it matters if we want sample sizes large enough that multiple coalescences 
occur in a single generation, becomes irrelevant if we can just simulate the entire population. 
As argued in \citep{BhaskarEtAl2014},  this is a parameter regime relevant to present-day human samples.  

We have presented the current approach as an alternative to jackknife approximations in 
achieving moment closure. The two approaches are, however, not mutually exclusive. 
Even though we have focused here on showing that closure could be reached within 
...
here allows one to draw a new lineage only when such a new lineage is required. 
It may not be necessary to reach closure at extremely high accuracy to achieve high accuracy
gains. 

\sgcomment{I think we are missing one concluding sentence here. }
