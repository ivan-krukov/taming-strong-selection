...
def Qs(io, no, ic, nc, N, s, cache, dense_cache = False):
    ...
    for ip in range(0, 2*n + 1):
        for io in range(0, n + 1):
            for nc in range(0, 2*n+1):
                for ic in range(0, nc+1):
                    mtx[ip, io] += Qs(io, n, ic, nc, N, s, cache, True) * hypergeom.pmf(ic, 2*n, ip, nc)
            
    # for nc in range(0, 2*n + 1):
    #     for ic in range(0, nc + 1):
    #         for io in range(0, n + 1):
    #             for no in range(0, n):
    #                 p = hypergeom.pmf(ic, 2*n, no, nc)
    #                 mtx[ic, io] += Qs(io, n, ic, nc, N, s, cache, dense_cache = True) * p
    return mtx, cache


# np.sum(m[:,0] * hypergeom.pmf(np.arange(0,21), 20, 0, 10))
