...
def basecases_t(io, no, ic, nc, s, t):
    if (no < 0) or (nc < 0) or (io < 0) or (ic < 0) or (io > no) or (ic > nc) or (no>0 and nc==0):
...
def Pf(io, no, ic, nc, s, N, t):
    ...
    if (io < 0) or (ic < 0) or (io > no) or (ic > nc) or (no < 0) or (nc < 0) or (no>0 and nc==0):
        return 0
    if t == 0:  # We will build a recursion over the number of successfully drawn offspring n_0.
...
def P0(io, no, ic, nc, s, N, max_t=1):
    ...
    if v is None: # Recursion on the number of
        ...
        bf = sum(Pf(io - 1, no - 1, ic - 1, nc - 1, s, N, t=i) for i in range(max_t + 1))
